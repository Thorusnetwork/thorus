...
const char * const BITCOIN_CONF_FILENAME = "thorus.conf";
const char * const BITCOIN_PID_FILENAME = "thorusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "thorus" is a composite category enabling all Thorus-related debug output
            if(ptrCategory->count(string("thorus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ThorusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ThorusCore
    // Mac: ~/Library/Application Support/ThorusCore
    // Unix: ~/.thoruscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ThorusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ThorusCore";
#else
    // Unix
    return pathRet / ".thoruscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty thorus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override thorus.conf
